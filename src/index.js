 ...
 * @returns 
 */
export async function walkLoadAndFlattenEnv(
  bottomDirPath=process.cwd(), topDirPath, 
  { direction='up', prefix="__ktr-srt__", walkToScope=false }={},
  options={ pathKeySize:5 }
) {

    const envarsObj = await walkAndLoadEnv(bottomDirPath, topDirPath, { direction, prefix, walkToScope }, options)
...
function flatten(envarsObj, direction) {
    ...
    const orderedEnvarObjs = orderedEnvarEntries.map(entry => entry[1]['vars'])
